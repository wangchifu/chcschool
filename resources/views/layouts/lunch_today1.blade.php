...
<?php
    $today = date('Y-m-d');
    $lunch_today = \App\LunchToday::find(1);
                                ...
                                if(k != 'dish'){
                                    ...
                                    for(var k1 in result[k]){
                                        ...
                                        if(q == 1){
                                            ...
                                            data += '<a class="nav-link" id="pills-profile-tab'+q+'" data-toggle="pill" href="#lunch_dish{{ $lunch_today->id }}_pills-profile'+q+'" role="tab" aria-controls="pills-profile" aria-selected="false" title="切換至 '+k1+'">'+k1+'</a>';
                                        }
                                        data += '</li>';
                                    }
                                    ...
                                    for(var k1 in result[k]){
                                        ...
                                        for(var k3 in result['dish'][result[k][k1]]){
                                            ...
                                            {{-- JS 無障礙 HM1240401C 與 CS2140401C 修正：完整 alt、rem 單位與開新視窗提示 --}}
                                        ...
                                        data += '<br>';
                                        data += '<a class="badge badge-info p-2 mt-2" href="https://fatraceschool.k12ea.gov.tw/frontend/search.html?school={{ $lunch_today->school_id }}&period='+$('#date{{ $lunch_today->id }}').val()+'" target="_blank" rel="noopener noreferrer" title="前往教育部智慧食材大平台查看詳細營養成份（另開新視窗）" aria-label="前往教育部智慧食材大平台查看詳細營養成份（另開新視窗）">查看詳細食材營養成份 <span class="sr-only">（另開新視窗）</span></a>';
                                        data += '</div>';
                                    }
                                    data += '</div></div>';
                                }
                        ...
                        <ul class="nav nav-pills mb-3" id="pills-tab" role="tablist">
                            ...
                            @foreach($v as $k1 => $v1)
                                ...
                                <li class="nav-item" role="presentation">
                                    ...
                                    @else
                                        <a class="nav-link" id="pills-profile-tab{{ $n }}" data-toggle="pill" href="#lunch_dish{{ $lunch_today->id }}_pills-profile{{ $n }}" role="tab" aria-controls="pills-profile" aria-selected="false" title="切換至 {{ $k1 }}">{{ $k1 }}</a>
                                    @endif
                                </li>
                            @endforeach
                        </ul>
